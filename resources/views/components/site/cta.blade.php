        ...
        <div class="relative px-6 py-12 sm:px-12 lg:py-16">
            <div class="max-w-2xl">
                ...
                <p class="mt-4 text-base leading-8 text-ink-300">
                    {{--
                        النطاق يُذكر كما ضبطه المالك، بلا ذيل يَعِد ببقية المملكة:
                        الوعد بمدينة لا تُغطّى يجلب طلبات تُرفض. ومن أراد السفر
                        فالسؤال الشائع «هل تغطي مدنًا خارج جدة؟» يجيبه.

                        وثلاث مدن حدٌّ للسطر لا للنطاق: الشريط سطرٌ واحد في أسفل
                        كل صفحة، وقائمةٌ طويلة فيه تزاحم الدعوة نفسها — والقائمة
                        كاملةً في صفحتي التواصل ونبذة.
                    --}}
                    @php
                        $areas = service_areas();
                        $shown = implode('، ', array_slice($areas, 0, 3)).(count($areas) > 3 ? ' وغيرها' : '');
                    @endphp

                    {{ $description ?: 'أرسل تفاصيل المناسبة — النوع والتاريخ والمكان — ويصلك عرض سعر دقيق خلال وقت قصير. التغطية متاحة في '.$shown.'.' }}
                </p>
