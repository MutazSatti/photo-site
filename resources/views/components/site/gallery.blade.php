...
@if ($items->isNotEmpty())
    ...
    >
        {{--
            كل بلاطة تأخذ نسبة صورتها نفسها فتظهر الصورة كاملةً بلا قصّ. ونسبة
            موحّدة لكل الشبكة لا تصلح هنا: الأعمال تخلط تجميعات نشر عمودية (9:16)
            بصور أفقية، فأي نسبة واحدة تقصّ نصف المجموعة.

            و items-start ضروري: عنصر الشبكة يتمدّد افتراضيًا إلى ارتفاع صفّه
            فيُلغي نسبة الأبعاد المطلوبة منه.
        --}}
        <div {{ $attributes->class(['grid grid-cols-1 items-start gap-4', $gridClass]) }}>
            @foreach ($items as $index => $item)
                <button
                    type="button"
                    x-on:click="show({{ $index }})"
                    style="aspect-ratio: {{ $item->aspectRatio() }}"
                    class="relative group overflow-hidden rounded-2xl bg-ink-100 dark:bg-ink-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500"
