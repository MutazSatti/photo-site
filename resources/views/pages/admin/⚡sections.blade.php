...
new #[Layout('layouts::admin', ['title' => 'الأقسام'])] class extends Component
{
    ...
    {
        $table = $this->editingType === 'section' ? 'sections' : 'categories';
        ...
        if ($this->editingType === 'section') {
            // تغيير رابط قسم رئيسي يكسر مسارات الموقع لأنها مقيّدة بأسماء ثابتة
            unset($payload['slug']);
            ...
            $exists = Category::where('section_id', $this->parentSectionId)
...
<div>
    <x-admin.page-header
        title="الأقسام"
        description="أقسام المعرض الأربعة وأقسامها الفرعية. النصوص هنا تظهر في الصفحات وفي البيانات المهيكلة."
    />
    ...
    <div class="grid gap-6 lg:grid-cols-3">
            ...
            @foreach ($this->sections as $section)
                <x-admin.card :padded="false">
                    <div class="flex flex-wrap items-center gap-3 px-5 py-4">
                        <span class="flex items-center justify-center rounded-xl size-10 bg-brand-50 text-brand-600 dark:bg-brand-950 dark:text-brand-400">
                    ...
                    @if ($section->categories->isNotEmpty() || $section->has_categories)
                        <div class="px-5 py-4 border-t bg-ink-50 border-ink-200 dark:border-ink-800 dark:bg-ink-950/40">
                            ...
                            <ul class="space-y-2">
                                ...
                                @empty
                                    <li class="py-3 text-xs text-center text-ink-400">لا توجد أقسام فرعية.</li>
                                @endforelse
                            </ul>
                        </div>
                    @endif
        ...
        <div>
            <div class="lg:sticky lg:top-20">
                @if ($editingType)
                    <x-admin.card :title="$editingType === 'section' ? 'تعديل قسم رئيسي' : ($editingId ? 'تعديل قسم فرعي' : 'قسم فرعي جديد')">
                        <form wire:submit="save" class="grid gap-5">
                            ...
                            <x-ui.field label="الاسم بالإنجليزية" :error="$errors->first('name_en')"
                                hint="يُنشر في البيانات المهيكلة كاسم بديل.">
                                <x-ui.input wire:model="name_en" dir="ltr" />
                            </x-ui.field>

                            <x-ui.field label="الرابط" required :error="$errors->first('slug')"
                                :hint="$editingType === 'section' ? 'روابط الأقسام الرئيسية ثابتة ولا تُعدَّل.' : null">
                                <x-ui.input wire:model="slug" dir="ltr" :disabled="$editingType === 'section'"
                ...
                @else
                    <x-admin.card>
                        <p class="py-8 text-sm text-center text-ink-500 dark:text-ink-400">
                            اختر قسمًا لتحريره، أو أضف قسمًا فرعيًا جديدًا.
                        </p>
                    </x-admin.card>
                @endif
            </div>
